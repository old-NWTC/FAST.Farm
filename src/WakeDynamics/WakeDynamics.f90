...
!..................................................................................................................................
   ...
   p%RootName  = TRIM(InitInp%RootName)//'.WD'
   
   
         
      
      ! Validate the initialization inputs
   call ValidateInitInputData( InitInp, ErrStat2, ErrMsg2 )
   ...
   p%DT            = InitInp%DT          
   p%NumPlanes     = InitInp%NumPlanes   
   p%NumRadii      = InitInp%NumRadii    
   p%dr            = InitInp%dr               
   p%C_NearWake    = InitInp%C_NearWake  
   p%C_vAmb_DMin   = InitInp%C_vAmb_DMin 
   p%C_vAmb_DMax   = InitInp%C_vAmb_DMax 
   p%C_vAmb_FMin   = InitInp%C_vAmb_FMin 
   p%C_vAmb_Exp    = InitInp%C_vAmb_Exp  
   p%C_vShr_DMin   = InitInp%C_vShr_DMin 
   p%C_vShr_DMax   = InitInp%C_vShr_DMax 
   p%C_vShr_FMin   = InitInp%C_vShr_FMin 
   p%C_vShr_Exp    = InitInp%C_vShr_Exp  
   p%k_vAmb        = InitInp%k_vAmb      
   p%k_vShr        = InitInp%k_vShr      
   p%Mod_WakeDiam  = InitInp%Mod_WakeDiam
   p%C_WakeDiam    = InitInp%C_WakeDiam  
   ...
   p%filtParam     = exp(-2*pi*p%dt*InitInp%f_c)
...
SUBROUTINE ValidateInitInputData( InitInp,  ErrStat, ErrMsg )
!..................................................................................................................................
      
      ! Passed variables:

   type(WD_InitInputType),   intent(in   )  :: InitInp                           !< Input data for initialization routine
 !  type(WD_InputFile),       intent(in)     :: InputFileData                     !< All the data in the WakeDynamics input file
   ...
   if (  InitInp%DT          <=  0.0)  call SetErrStat ( ErrID_Fatal, 'DT must be greater than zero.', ErrStat, ErrMsg, RoutineName )  
   if (  InitInp%NumPlanes   <   2  )  call SetErrStat ( ErrID_Fatal, 'Number of wake planes must be greater than one.', ErrSTat, ErrMsg, RoutineName )
   if (  InitInp%NumRadii    <   2  )  call SetErrStat ( ErrID_Fatal, 'Number of radii in the radial finite-difference grid must be greater than one.', ErrSTat, ErrMsg, RoutineName )
   if (  InitInp%dr          <=  0.0)  call SetErrStat ( ErrID_Fatal, 'dr must be greater than zero.', ErrStat, ErrMsg, RoutineName ) 
   if (  InitInp%f_c         <=  0.0)  call SetErrStat ( ErrID_Fatal, 'f_c must be greater than or equal to zero.', ErrStat, ErrMsg, RoutineName ) 
   if (  InitInp%C_NearWake  <= -1.0)  call SetErrStat ( ErrID_Fatal, 'C_NearWake must be greater than -1.0.', ErrStat, ErrMsg, RoutineName ) 
   if (  InitInp%k_vAmb      <   0.0)  call SetErrStat ( ErrID_Fatal, 'k_vAmb must be greater than or equal to zero.', ErrStat, ErrMsg, RoutineName ) 
   if (  InitInp%k_vShr      <   0.0)  call SetErrStat ( ErrID_Fatal, 'k_vShr must be greater than or equal to zero.', ErrStat, ErrMsg, RoutineName ) 
   if (  InitInp%C_vAmb_DMin <   0.0)  call SetErrStat ( ErrID_Fatal, 'C_vAmb_DMin must be greater than or equal to zero.', ErrStat, ErrMsg, RoutineName ) 
   if (  InitInp%C_vAmb_DMax <=  InitInp%C_vAmb_DMin)  call SetErrStat ( ErrID_Fatal, 'C_vAmb_DMax must be greater than C_vAmb_DMin.', ErrStat, ErrMsg, RoutineName ) 
   if ( (InitInp%C_vAmb_FMin <   0.0)  .or. (InitInp%C_vAmb_FMin > 1.0) ) call SetErrStat ( ErrID_Fatal, 'C_vAmb_FMin must be greater than or equal to zero and less than or equal to 1.0.', ErrStat, ErrMsg, RoutineName ) 
   if (  InitInp%C_vAmb_Exp  <=  0.0)  call SetErrStat ( ErrID_Fatal, 'C_vAmb_Exp must be greater than zero.', ErrStat, ErrMsg, RoutineName ) 
   if (  InitInp%C_vShr_DMin <   0.0)  call SetErrStat ( ErrID_Fatal, 'C_vShr_DMin must be greater than or equal to zero.', ErrStat, ErrMsg, RoutineName ) 
   if (  InitInp%C_vShr_DMax <=  InitInp%C_vShr_DMin)  call SetErrStat ( ErrID_Fatal, 'C_vShr_DMax must be greater than C_vShr_DMin.', ErrStat, ErrMsg, RoutineName ) 
   if ( (InitInp%C_vShr_FMin <   0.0)  .or. (InitInp%C_vShr_FMin > 1.0) ) call SetErrStat ( ErrID_Fatal, 'C_vShr_FMin must be greater than or equal to zero and less than or equal to 1.0.', ErrStat, ErrMsg, RoutineName ) 
   if (  InitInp%C_vShr_Exp <=   0.0)  call SetErrStat ( ErrID_Fatal, 'C_vShr_Exp must be greater than zero.', ErrStat, ErrMsg, RoutineName ) 
   if (.not. ((InitInp%Mod_WakeDiam == 1) .or. (InitInp%Mod_WakeDiam == 2) .or. (InitInp%Mod_WakeDiam == 3) ) ) call SetErrStat ( ErrID_Fatal, 'Mod_WakeDiam must be equal to 1, 2, or 3.', ErrStat, ErrMsg, RoutineName ) 
   if ( (InitInp%C_WakeDiam <=   0.0)  .or. (InitInp%C_WakeDiam >= 1.0) ) call SetErrStat ( ErrID_Fatal, 'C_WakeDiam must be greater than zero and less than 1.0.', ErrStat, ErrMsg, RoutineName ) 
...
subroutine WD_TEST_Init_BadData(errStat, errMsg)
   ...
   InitInp%dr             = 0.0_ReKi
   InitInp%f_c            = 0.0
   InitInp%C_NearWake     = -1.001
   InitInp%C_vAmb_DMin    = -0.01
   InitInp%C_vAmb_DMax    = -3
   InitInp%C_vAmb_FMin    = 2
   InitInp%C_vAmb_Exp     = 0
   InitInp%C_vShr_DMin    = -.1
   InitInp%C_vShr_DMax    = -3
   InitInp%C_vShr_FMin    = -1
   InitInp%C_vShr_Exp     = -1
   InitInp%k_vAmb         = -2
   InitInp%k_vShr         = -2
   InitInp%Mod_WakeDiam   = 0
   InitInp%C_WakeDiam     = 0
...
subroutine WD_TEST_SetGoodInitInpData(InitInp)
   ...
   InitInp%NumRadii       = 2
   InitInp%dr             = 0.1_ReKi
   InitInp%f_c            = 0.03333333333333
   InitInp%C_NearWake     = 2
   InitInp%C_vAmb_DMin    = 0
   InitInp%C_vAmb_DMax    = 2
   InitInp%C_vAmb_FMin    = 0
   InitInp%C_vAmb_Exp     = 1
   InitInp%C_vShr_DMin    = 2
   InitInp%C_vShr_DMax    = 11
   InitInp%C_vShr_FMin    = .035
   InitInp%C_vShr_Exp     = .4
   InitInp%k_vAmb         = .07
   InitInp%k_vShr         = .0178
   InitInp%Mod_WakeDiam   = 1
   InitInp%C_WakeDiam     = .95
...
subroutine WD_TEST_Init_GoodData(errStat, errMsg)
   ...
   type(WD_InitOutputType)      :: initOut                         !< Input data for initialization routine
      ...
      ! Set up the initialization inputs
   call WD_TEST_SetGoodInitInpData(InitInp)
